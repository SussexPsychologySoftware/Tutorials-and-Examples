...
## Using Embedded Data (Creating conditions)

## Longitudinal studies using workflows

## Integrating jsPsych and Qualtrics
The current version of jsPsych doesn't work in Qualtrics directly, so we'll present two options here for running your jsPsych task within your Qualtrics survey.
...
## Integrating JavaScript with Qualtrics
...
- jQuery is a library designed to make JavaScript easier to write. jQuery is available in Qualtrics, although isn't accessed by the `$` as usual, but using `jQuery()` in place of this.
...
Note the following is not applicable to jsPsych code at present.
